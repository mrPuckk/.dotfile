...
/**
 Represents information about the CPU, including host details, CPU load data,
 and a command string.

 - Parameters:
   - host: A `host_t` object representing the host information.
   - count: A `mach_msg_type_number_t` indicating the count of CPU load data.
   - load: A `host_cpu_load_info_data_t` object containing the current CPU load
 information.
   - prev_load: A `host_cpu_load_info_data_t` object containing the previous CPU
 load information.
   - has_prev_load: A boolean flag indicating whether previous load information
 is available.
   - command: A character array of size 256 representing a command associated
 with the CPU.

 This struct is designed for managing CPU-related information in an Apple
...
static inline void cpu_update(struct cpu *cpu) {
  ...
  if (cpu->has_prev_load) {
    ...
    char *start = strstr(line, FILTER_PATTERN); // Ignore app with ".apple.com"
